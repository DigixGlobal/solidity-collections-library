...
contract('UintIteratorInteractive', function () {
  ...
  describe('list_uints (from start)', function () {
    it('[collection is not empty, 0<count<=size of collection] returns correct items', async function () {
      const res = await exampleInteractive.list_uints_collection.call(bN(6), true);
    ...
    it('[collection is not empty, count=0] returns empty array', async function () {
      const res = await exampleInteractive.list_uints_collection.call(bN(0), true);
    ...
    it('[collection is empty] returns empty array', async function () {
      ...
      const res = await exampleInteractive.list_uints_collection.call(bN(2), true);
      assert.deepEqual(res.length, 0);
    });
  });

  describe('list_uints_from (from start)', function () {
    it('[item is not the last, count <= items remaining] returns correct items', async function () {
      const res = await exampleInteractive.list_uints_collection_from.call(exampleUints[3], bN(2), true);
    ...
    it('[item is not the last, count > items remaining] returns all items remaining', async function () {
      const res = await exampleInteractive.list_uints_collection_from.call(exampleUints[4], bN(3), true);
    ...
    it('[item is the second last, count > 0] returns just the item', async function () {
      const res = await exampleInteractive.list_uints_collection_from.call(exampleUints[5], bN(3), true);
    ...
    it('[item is the last, count > 0] returns empty array', async function () {
      const res = await exampleInteractive.list_uints_collection_from.call(exampleUints[6], bN(3), true);
      // assert.deepEqual(res[0], exampleUints[6]);
      assert.deepEqual(res.length, 0);
    });
    it('[count = 0] returns empty array', async function () {
      const res = await exampleInteractive.list_uints_collection_from.call(exampleUints[4], bN(0), true);
      assert.deepEqual(res.length, 0);
    });
    it('[item does not exist] returns empty array', async function () {
      const res = await exampleInteractive.list_uints_collection_from.call(bN(100), bN(1), true);
      assert.deepEqual(res.length, 0);
    });
    it('[collection is empty] returns empty array', async function () {
      await exampleStorage.remove_all_data_in_uints_collection();
      const res = await exampleInteractive.list_uints_collection_from.call(exampleUints[5], bN(1), true);
      assert.deepEqual(res.length, 0);
    });
  });

  describe('list_uints (from end)', function () {
    it('[collection is not empty, 0<count<=size of collection] returns correct items', async function () {
      const res = await exampleInteractive.list_uints_collection.call(bN(6), false);
      assert.deepEqual(res[0], exampleUints[6]);
      assert.deepEqual(res[1], exampleUints[5]);
    ...
    it('[collection is not empty, count=0] returns empty array', async function () {
      const res = await exampleInteractive.list_uints_collection.call(bN(0), false);
      assert.deepEqual(res.length, 0);
    });
    it('[collection is not empty, count>size of collection] returns all the items', async function () {
      const res = await exampleInteractive.list_uints_collection.call(bN(6), false);
    ...
    it('[collection is empty] returns empty array', async function () {
      await exampleStorage.remove_all_data_in_uints_collection();
      const res = await exampleInteractive.list_uints_collection.call(bN(2), false);
      assert.deepEqual(res.length, 0);
    });
  });

  describe('list_uints_from (from end)', function () {
    it('[item is not the last, count <= items remaining] returns correct items', async function () {
      const res = await exampleInteractive.list_uints_collection_from.call(exampleUints[3], bN(2), false);
    ...
    it('[item is not the first, count > items remaining] returns all items remaining', async function () {
      const res = await exampleInteractive.list_uints_collection_from.call(exampleUints[3], bN(3), false);
      assert.deepEqual(res[0], exampleUints[2]);
      assert.deepEqual(res[1], exampleUints[1]);
      assert.deepEqual(res.length, 2);
    });
    it('[item is the second, count > 0] returns just the first item', async function () {
      const res = await exampleInteractive.list_uints_collection_from.call(exampleUints[2], bN(3), false);
      assert.deepEqual(res[0], exampleUints[1]);
      assert.deepEqual(res.length, 1);
    });
    it('[item is the first, count > 0] returns empty array', async function () {
      const res = await exampleInteractive.list_uints_collection_from.call(exampleUints[1], bN(3), false);
      // console.log('res = ', res);
      // assert.deepEqual(res[0], exampleUints[6]);
      assert.deepEqual(res.length, 0);
    });
    it('[count = 0] returns empty array', async function () {
      const res = await exampleInteractive.list_uints_collection_from.call(exampleUints[6], bN(0), false);
      assert.deepEqual(res.length, 0);
    });
    it('[item does not exist] returns empty array', async function () {
      const res = await exampleInteractive.list_uints_collection_from.call(bN(100), bN(1), false);
      assert.deepEqual(res.length, 0);
    });
    it('[collection is empty] returns empty array', async function () {
      await exampleStorage.remove_all_data_in_uints_collection();
      const res = await exampleInteractive.list_uints_collection_from.call(exampleUints[5], bN(1), false);
      assert.deepEqual(res.length, 0);
    });
  });
});
